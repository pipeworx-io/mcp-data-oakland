 ...
 * Use `datasets` -> `metadata` -> `query` (SoQL).
 */


const BASE = 'https://data.oaklandca.gov';
const UA = 'pipeworx-mcp-data-oakland/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'datasets',
    description: 'Search the Oakland Open Data catalog of open datasets by keyword. Returns each dataset\'s resource_id, name, description, category and update date — pass the resource_id to query/metadata.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'query',
    description: 'Run a Socrata SoQL query against a Oakland Open Data dataset by resource_id (e.g. "ym6k-rx7a"). Filter with where/select/group/order (SoQL clauses, without the leading $) plus limit/offset. Returns matching rows as JSON.',
    inputSchema: {
      type: 'object',
      properties: {
        resource_id: { type: 'string', description: 'Dataset id, e.g. "ym6k-rx7a" (from datasets).' },
  ...
  {
    name: 'metadata',
    description: 'Get a Oakland Open Data dataset\'s schema + metadata (columns, types, row count, category, last-updated) by resource_id, e.g. "ym6k-rx7a".',
    inputSchema: {
      type: 'object',
      properties: { resource_id: { type: 'string', description: 'Dataset id, e.g. "ym6k-rx7a".' } },
      required: ['resource_id'],
    },
  },
];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'query': {
      const id = reqStr(args, 'resource_id', '"ym6k-rx7a"');
      const p = new URLSearchParams();
    ...
    case 'metadata':
      return socrataGet(`/api/views/${reqStr(args, 'resource_id', '"ym6k-rx7a"')}.json`);
    default:
...
async function socrataGet(path: string): Promise<unknown> {
  ...
  if (!res.ok) throw new Error(`data.oaklandca.gov: ${res.status}`);
  return res.json();
}
